...
/**
 * @title The OneInchV4TokenSwapper token swapper.
 * @notice The OneInchV4TokenSwapper contract allows to leverage
 * the 1Inch dex aggregator.
 * @author Alessandro Sanino (@saniales)
 */
contract OneInchV4TokenSwapper is ITokenSwapper {
    using SafeERC20 for IERC20;

    /**
     * @notice The address of the 1Inch router contract.
     */
    address immutable AGGREGATION_ROUTER_ONE_INCH_V4;
    ...
    /**
     * @notice Creates a new instance of the OneInchV4TokenSwapper contract.
    ...
    constructor(address _aggregationRouterOneInch, IERC20 _destinationToken) {
        AGGREGATION_ROUTER_ONE_INCH_V4 = _aggregationRouterOneInch;
    ...
    function _swapUsingOneInchSwap(bytes calldata _oneInchSwapData) internal {
        ...
        if (desc.srcToken != desc.dstToken && desc.dstToken == DESTINATION_TOKEN) {
            require(desc.srcToken.approve(AGGREGATION_ROUTER_ONE_INCH_V4, desc.amount), "OneInchV4TokenSwapper.swap: Must be able for the contract to approve transfer to the router");

            (bool callSuccess, bytes memory routerData) = address(AGGREGATION_ROUTER_ONE_INCH_V4).call(_oneInchSwapData);
            require(callSuccess, "OneInchV4TokenSwapper.swap: Must be able to call the aggregation router to perform the swap");

            (uint256 returnAmount, ) = abi.decode(routerData, (uint256, uint256));
            require(returnAmount >= desc.minReturnAmount, "OneInchV4TokenSwapper.swap: Must be able to swap at least minOut");
        }
    }
    ...
    function _swapUsingOneInchUnoswap(bytes calldata _oneInchSwapData) internal {
        ...
        if (srcToken != DESTINATION_TOKEN) {
            require(srcToken.approve(AGGREGATION_ROUTER_ONE_INCH_V4, amount), "OneInchV4TokenSwapper.unoswap: Must be able for the contract to approve transfer to the router");

            uint256 balanceBeforeSwap = DESTINATION_TOKEN.balanceOf(address(this));

            (bool callSuccess, bytes memory routerData) = address(AGGREGATION_ROUTER_ONE_INCH_V4).call(_oneInchSwapData);
            require(callSuccess, "OneInchV4TokenSwapper.unoswap: Must be able to call the aggregation router to perform the unoswap");

            uint256 balanceAfterSwap = DESTINATION_TOKEN.balanceOf(address(this));
            require(balanceBeforeSwap < balanceAfterSwap, "OneInchV4TokenSwapper.unoswap: Balance of DESTINATION_TOKEN must be higher after the swap");

            (uint256 returnAmount) = abi.decode(routerData, (uint256));
            require(returnAmount >= minReturnAmount, "OneInchV4TokenSwapper.unoswap: Must be able to unoswap at least minOut");
        }
    }
    ...
    function _beforeSwap(bytes calldata _oneInchSwapData) internal override view {
        ...
        if (functionSignature == ONE_INCH_SWAP_SIGNATURE) {
            (, OneInchSwapDescription memory desc,) = abi.decode(_oneInchSwapData[4:], (address, OneInchSwapDescription, bytes));
            require(address(desc.dstToken) == address(DESTINATION_TOKEN), "OneInchV4TokenSwapper: Destination token must always be equal to the one specified in the contract");
            require(desc.srcReceiver == address(this), "OneInchV4TokenSwapper: data from 1Inch API must set this contract as source of the swap");
            require(desc.dstReceiver == address(this), "OneInchV4TokenSwapper: data from 1Inch API must set this contract as destination of the swap");
            require(desc.minReturnAmount > 0, "OneInchV4TokenSwapper: minimum return amount must be > 0");    
        } else if (functionSignature == ONE_INCH_UNOSWAP_SIGNATURE) {
            (, , uint256 minReturnAmount, ) = abi.decode(_oneInchSwapData[4:], (IERC20, uint256, uint256, bytes));
            require(minReturnAmount > 0, "OneInchV4TokenSwapper: minimum return amount must be > 0");
        } else {
            revert("Invalid function signature");
        }
    }
